...
exports.getRunOneTestcafeFileCommand = ({
  ...
  const testcafeOptions =
    '--assertion-timeout 5000 --selector-timeout 15000 -c 4'
...
exports.getRunAllTestcafeFilesCommand = config => {
  ...
  const testFileNames = fs.readdirSync(path.join(fileDir, '../testcafe'))
                          .filter(dirName => dirName !== "helpers")
  const command = testFileNames.map(testFileName =>
    exports.getRunOneTestcafeFileCommand(
...
*/
exports.getRunAllTestcafeFilesCommand = config => {
  return exports.getRunOneTestcafeFileCommand(config, { file: '' })
}
