...
import { isSyncronizableProvider } from '../../domain/localProvider'
...
const OfferForm = props => {
  ...
  useEffect(() => {
    ...
    if (offer) {
      ...
      setHasSyncStocks(isSyncronizableProvider(offer))
      setReadOnlyFields([
        'type',
        'musicType',
        'musicSubType',
  ...
  useEffect(() => {
    ...
    if (venuesToShow.length) {
      if (
        formValues.venueId &&
        venuesToShow.length &&
        !venuesToShow.find(venue => venue.id === formValues.venueId)
      ) {
