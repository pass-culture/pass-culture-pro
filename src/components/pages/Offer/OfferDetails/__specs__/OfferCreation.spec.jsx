...
describe('offerDetails - Creation', () => {
  ...
  const setOfferValues = async (values) => {
    const fieldLabels = {
      ...
      visa: { label: 'Visa d’exploitation', exact: true },
  ...
  describe('render when creating a new offer', () => {
    ...
    describe('when selecting an offer type', () => {
      ...
      xit('should display an error message when offer type is offline only and offerer has no physical venue', async () => {
        // Given
        venues = [
          {
            id: 'ABCD',
        ...
        // Then
        expect(screen.queryByText(venues[0].name)).not.toBeInTheDocument()
        const errorMsg = await screen.findByText('Ce champ est obligatoire.')
